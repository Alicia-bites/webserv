...
def _test_count(path: str = "./tests/http/main.cpp") -> int:
    """
...
def test_http_module_full(test_index):
    """
    ...
    if not os.path.exists(BINARY_PATH):
        try:
            ...
            os.chdir("../../")
            raise Exception("Failed to compile HTTP module")

    # Run all tests (if return code is 9, abort the test)
    try:
        output = _memcheck(BINARY_PATH, test_index)
    except subprocess.CalledProcessError:
        raise Exception("Failed to execute config module")
    finally:
